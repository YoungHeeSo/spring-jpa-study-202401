...
class EmployeeRepositoryTest {
    ...
    void findOneTest() {
        ...
        Employee employee = employeeRepository.findById(id)
                // orElseThrow : find 결과가 null일 경우 예외가 발생하도록 예외처리
                .orElseThrow(
        ...
        /*
            select 쿼리가 실행이 안됬는데 조회가 된 이유는
            영속성 컨텍스트라는 개념이 JPA 에 존재하는데
            INSERT 직후 SELECT 를 하면 두 개의 쿼리가 하나의 트랜잭션으로 연결되어
            INSERT한 내용을 메모리에서 불러오기 때문에 DB를 조회하지 않고 성능최적화를 함
         */
